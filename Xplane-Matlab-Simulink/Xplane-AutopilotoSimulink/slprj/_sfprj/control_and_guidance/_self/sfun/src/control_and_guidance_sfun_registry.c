...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_control_and_guidance_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_control_and_guidance_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_control_and_guidance_autoinheritance_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  ...
  if (strcmp(machineName, "control_and_guidance") == 0) {
    const mxArray *newRhs[3] = { NULL, NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_control_and_guidance_autoinheritance_info(nlhs,plhs,3,newRhs);
  }
...
{
  extern unsigned int sf_control_and_guidance_get_eml_resolved_functions_info
  ...
  if (strcmp(machineName, "control_and_guidance") == 0) {
    ...
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_control_and_guidance_get_eml_resolved_functions_info(nlhs,plhs,2,
      newRhs);
  }
...
extern unsigned int sf_control_and_guidance_method_dispatcher(SimStruct *S,
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"control_and_guidance")) {
    return(sf_control_and_guidance_method_dispatcher(simstructPtr,
...
extern void control_and_guidance_terminator(void);
void sf_machine_global_terminator(void)
{
  if (sfMachineGlobalTerminatorCallable) {
    ...
    control_and_guidance_terminator();
    sf_debug_terminate();
  }
...
extern void control_and_guidance_initializer(void);
extern void control_and_guidance_register_exported_symbols(SimStruct* S);
extern void control_and_guidance_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  control_and_guidance_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      control_and_guidance_debug_initialize();
    }

    control_and_guidance_initializer();
  }
